...
def octet_reverser(payload):
    ...
    # octets = [payload[i:i+8] for i in range(0, len(payload), 8)]
    # reversed_octets = [octet[::-1] for octet in octets]
    return reversedstring


#This was the problem somehow
def bit_stuffer(data):
    ...
    for index, bit in enumerate(data):
        ...
        if count == 5:
            # if index + 1 < len(data) and data[index + 1] == '1': #for some reason this logic is not applied? i guess for speed?
            stuffed_data += '0'
            count = 0
